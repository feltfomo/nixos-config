// Define our URLs
const BucketURL =
  "https://extensions-storage.socalifornian.live/beautiful-lyrics"; // This is replaced by the build-script
const LatestVersionURL =
  "https://extensions.socalifornian.live/version/beautiful-lyrics"; // This is replaced by the build-script
...
const UpdateVersion = (toVersion) => {
  ...
  if (
    ...
    QueuedVersionImports.includes(toVersion)
  ) {
    return;
  } else if (importing) {
  ...
  const fromVersionInformation =
    fromVersion === undefined ? undefined : GetVersionInformation(fromVersion);
  const toVersionInformation = GetVersionInformation(toVersion);
  const versionDistance =
    fromVersionInformation === undefined
      ? undefined
      : {
  ...
  {
    import(`${BucketURL}${encodeURIComponent(`@${toVersion}.mjs`)}`).then(
      (module) => {
