...
in
{
  ...
  let
    ...
    mkProfileWrapper = profile:
      let
        extraPathStr = lib.concatStringsSep ":" (profile.extra_path or []);
        pathExport = lib.optionalString (extraPathStr != "")
          "export PATH=\"${extraPathStr}:$PATH\"\n";
      in
        pkgs.writeShellScriptBin "zeditor-${profile.name}" ''
          export ZED_DEVSHELL="${profile.devshell}"
          export XDG_DATA_HOME="/home/${vars.username}/.local/share/zed-${profile.name}"
  ...
  flake.nixosModules.zed = { pkgs, lib, ... }:
  let
    sharedSettings = {
      theme = {
        mode = "dark";
  ...
  in
  {
    environment.systemPackages = [
      ...
      self.packages.${pkgs.stdenv.hostPlatform.system}."zed-${p.name}"
    ) profiles;
